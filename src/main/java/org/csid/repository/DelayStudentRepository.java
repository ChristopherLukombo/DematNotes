...
public interface DelayStudentRepository extends JpaRepository<DelayStudent, Long> {
    ...
    @Query("select delayStudent from DelayStudent delayStudent left join fetch delayStudent.students where delayStudent.startDate<=:dateTime and delayStudent.endDate<=:dateTime")
    List<DelayStudent> findAllAbsencesByPeriod(@Param("dateTime") ZonedDateTime dateTime);
