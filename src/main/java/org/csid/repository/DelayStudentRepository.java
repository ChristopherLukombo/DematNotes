...
public interface DelayStudentRepository extends JpaRepository<DelayStudent, Long> {
    ...
    @Query("select delayStudent from DelayStudent delayStudent left join fetch delayStudent.students where delayStudent.startDate>=:periodDate")
    List<DelayStudent> findAllDelayStudentsByPeriod(@Param("periodDate") ZonedDateTime periodDate);
